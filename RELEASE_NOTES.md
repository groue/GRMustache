...
- The `each` filter let templates access array indexes, dictionary keys, and more. [Documentation](Guides/standard_library.md#each).
...
`each` is inspired by [Handlebars.js](http://handlebarsjs.com).
`zip` is inspired by [issue #80](https://github.com/groue/GRMustache/issues/80).
